...
from singularity.paths import LANGUAGES, TEMP, BINARIES
from singularity.utils import vprint, humanbytes, request_webpage

PYTHON_GIT = 'https://github.com/Aveeryy/Singularity/archive/refs/heads/main.zip'

def selfupdate(mode='git'):
    'Self-update Singularity from the latest release'

    if sys.argv[0].endswith('.py'):
        # Update python package
        installation_path = os.path.dirname(sys.argv[0]).removesuffix('singularity')
        ...
        if mode == 'git':
            ...
            for item in os.listdir(f'{TEMP}Singularity-main/'):
                shutil.move(f'{TEMP}Singularity-main/{item}', installation_path)
            # Clean up
            os.rmdir(f'{TEMP}Singularity-main/')
